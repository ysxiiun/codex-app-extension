...
它的目标不是替代 Codex App，也不是修改应用包体，而是在运行时为 Codex App 补充一些更贴近实际工作流的体验增强。当前版本主要解决宽屏空间利用、macOS 全屏遮挡、中文输入法回车误发送、长文本发送体验、Tab 制表符输入等问题；后续也可以继续承载更多 Codex App 的增强注入能力。
...
- Codex App 右侧出现悬浮栏或主聊天区变窄时，会按实际可用区域动态收敛，并至少保留左右各 `50px` 留白。
...
脚本会读取当前配置文件，显示每一项当前值，直接按 Enter 会保留当前值；对布尔开关可以输入 `y` / `n`、`true` / `false`、`on` / `off` 或 `1` / `0` 切换。运行完成后，配置文件会补齐当前版本支持的全量配置项。

主题增强的配色和排版属于复杂配置，`config.sh` 只会补齐 `themeEnhancementColors` 与 `themeEnhancementTypography` 的完整结构，不会逐个颜色交互修改。开启 `themeEnhancement` 后，如需调整标题、引用、行内代码或加粗样式，请直接编辑配置文件中的这两个对象。
...
第一次运行注入脚本或配置脚本时，如果配置文件不存在，会自动生成默认配置：
...
- `contentMaxWidth`：正文、输入框、宽 Markdown、代码块的统一最大宽度；当主聊天区可用宽度不足时，会自动收敛到当前可用区域，并至少保留左右各 `50px` 留白。
...
- `themeEnhancement`：是否启用“Markdown 主题增强”，默认关闭；开启后为标题、列表 marker、行内代码、引用块和加粗补充结构色。
- `themeEnhancementColors`：主题增强色板。当前默认值适配 Codex App 的 `codex` dark 主题；如果使用其他主题，需要根据实际背景色、正文色和代码主题自行调整这些颜色。
- `themeEnhancementTypography`：主题增强排版覆盖项，默认不覆盖 Codex 原生排版；字段值设为 `null` 表示保持原生效果。

`themeEnhancementColors` 字段说明：

| 字段 | 影响内容 |
|---|---|
| `orderedListMarker` | Markdown 有序列表序号，例如 `1.`、`2.` |
| `unorderedListMarker` | Markdown 无序列表圆点，例如 `-` / `*` 渲染后的 bullet |
...
| `headingText` | Markdown 标题文字颜色，例如 `#` / `##` / `###` |
| `strongText` | Markdown 加粗文字颜色，例如 `**重点**` |

`themeEnhancementTypography` 字段说明：

| 字段 | 影响内容 |
|---|---|
| `strongFontWeight` | Markdown 加粗文字字重，例如 `"800"`；`null` 表示不覆盖 Codex 原生字重 |
| `strongFontSize` | Markdown 加粗文字字号，例如 `"1.05em"`；`null` 表示不覆盖 Codex 原生字号 |
...
宽度增强会优先按 Codex 主聊天区域的实际宽度计算；如果配置的最大宽度超过当前可用区域，会自动收敛，并保留至少 `100px` 总留白。
...
- `config.minSideGutter` / `config.minTotalSidePadding`：动态宽度适配时保留的最小单侧 / 总留白。
- `layoutWidthState`：当前宽屏变量的计算来源，包括主聊天参考容器、可用宽度、有效留白、最终宽度表达式和右侧悬浮栏候选。
...
- `themeEnhancementTypography`：当前生效的主题增强排版覆盖项。
