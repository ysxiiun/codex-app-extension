...
You MUST dispatch sub-agents using Codex sub-agent dispatch for every unit, whatever the
...
3. Dispatch according to strategy, via Codex sub-agent dispatch:
   ...
   Platform spawn rule: Use Codex sub-agent delegation where available; pass the full task card in the prompt.
