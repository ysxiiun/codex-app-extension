...
description: Pre-implementation design skill. Use when the user runs $ec-brainstorming, asks to brainstorm/explore/shape a design, or when a startup project needs its first direction after ec-init. Turns an idea into a confirmed design doc through one-question-at-a-time dialogue. Hard gate — no implementation before the user approves the design.
---
...
1. **Start task now** — invoke `$ec-workflow` with the design topic as the
