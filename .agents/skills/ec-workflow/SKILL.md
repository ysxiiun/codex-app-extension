...
3. **State check + Intent routing.** Read the hook-injected breadcrumbs (`[current-task:X]`,
   ...
   `python3 .codex/hooks/easy_coding_state.py list-tasks --agent <agent-id>`.
...
4. **New task.** When creating a task (from step 3), create
   ...
   `python3 .codex/hooks/easy_coding_state.py create-task --session-file <P> --task-id <MM-DD-task-slug> --type <feature|bugfix|refactor|perf|doc|analysis|report|workflow> --title "<one-line summary>" --agent <agent-id>`.
...
2. Claim the selected task through the state API:
   `python3 .codex/hooks/easy_coding_state.py claim-task --session-file <P> --task-id <task-b-id> --agent <agent-id>`.
...
- **At confirmation-required stage completion** request the legal target immediately:
  `python3 .codex/hooks/easy_coding_state.py request-transition --session-file <P> --stage <STAGE> --agent <agent-id> --reason "<why this edge is ready>"`.
...
- **On an automatic edge** call:
  `python3 .codex/hooks/easy_coding_state.py auto-transition --session-file <P> --stage <TARGET> --agent <agent-id>`.
...
- **On user confirmation** read the current task's pending edge and call:
  `python3 .codex/hooks/easy_coding_state.py confirm-transition --session-file <P> --stage <STAGE> --agent <agent-id>`.
...
- **On Other feedback or a changed outcome** call:
  `python3 .codex/hooks/easy_coding_state.py cancel-transition --session-file <P> --agent <agent-id>`
...
option, call:
`python3 .codex/hooks/easy_coding_state.py handoff-task --session-file <P> --agent <agent-id> --summary "<dense context: plan shape, key decisions, user emphases>"`.
