---
name: ec-init
description: One-time project knowledge initialization after the easy-coding CLI install. Use when the user runs /ec-init or hook context shows [easy-coding:init-required]. Idempotent. Detects startup vs iterative projects automatically and generates SOUL, RULES, ABSTRACT, and TEST_STRATEGY.
---
...
2. Otherwise run
   `python3 .claude/hooks/easy_coding_state.py snapshot --agent <agent-id>`
...
gaps were fixed), run
`python3 .claude/hooks/easy_coding_state.py project-init-complete --session-file <P> --agent <agent-id>`.
