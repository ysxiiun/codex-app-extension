...
description: Task and session panel for Easy Coding. Use when the user runs /ec-task-management, asks to see/create/continue/take over tasks, or asks to view or change the current session confirm mode.
---
...
A bare `/ec-task-management` invocation means "show the full task and session
...
`python3 .claude/hooks/easy_coding_state.py claim-task --session-file <P> --task-id <task-id> --agent <agent-id>`.
...
`python3 .claude/hooks/easy_coding_state.py create-task --session-file <P> --task-id <MM-DD-short-name> --type <feature|bugfix|refactor|perf|doc|analysis|report|workflow> --title "<one-line summary>" --agent <agent-id> --no-set-current`.

If the user explicitly wants to start the task now, omit `--no-set-current`; otherwise leave
the current workflow pointer untouched and tell them to run `/ec-workflow`
when ready.
...
`python3 .claude/hooks/easy_coding_state.py set-confirm-mode --session-file <P> --mode <approve|guard|lite|auto> --agent <agent-id>`.

Restore project configuration through:
`python3 .claude/hooks/easy_coding_state.py clear-confirm-mode --session-file <P> --agent <agent-id>`.
