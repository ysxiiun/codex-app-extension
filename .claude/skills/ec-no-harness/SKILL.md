---
name: ec-no-harness
description: Disable Easy Coding Harness orchestration for the current agent session and use native agent behavior. Use only when the user explicitly invokes /ec-no-harness or explicitly asks to restore Easy Coding after this bypass.
---
...
`python3 .claude/hooks/easy_coding_state.py disable-harness --session-file <P> --agent <agent-id>`
...
`python3 .claude/hooks/easy_coding_state.py enable-harness --session-file <P> --agent <agent-id>`
